...
/// </remarks>
class PNPDWG2DANNO_API AcPpOrthoAnnotation : public AcPpAnnotation
{
public:
    ACRX_DECLARE_MEMBERS(AcPpOrthoAnnotation);
    ...
    AcPpOrthoAnnotation(const AcDbObjectId& annotationId = AcDbObjectId::kNull);

    /// <summary>
    /// Destructor. 
    /// </summary>
    virtual ~AcPpOrthoAnnotation();

    // Coordinate system
    //
    const AcGeVector3d &    getCoordOffset      () const;
    void                    setCoordOffset      (const AcGeVector3d &value);
    double                  getCoordRotation    () const;
    void                    setCoordRotation    (double value);
...
private:
    AcPpOrthoImpAnnotation* implementation() const;
