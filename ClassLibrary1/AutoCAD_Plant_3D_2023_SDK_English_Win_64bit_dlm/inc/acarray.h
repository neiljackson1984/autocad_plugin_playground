...
// The maxLength() method returns how many items can fit in the array before the
// index value overflows.
//
...
AcArray<T,R>::setGrowLength(int glen)
{
    ...
    AC_ARRAY_ASSERT(glen <= maxLength());
    mGrowLen = glen;
    return *this;
}
...
AcArray< T, R > ::AcArray(int physicalLength, int growLength) : mGrowLen(growLength)
{
    ...
    AC_ARRAY_ASSERT(mGrowLen <= maxLength());
    AC_ARRAY_ASSERT(physicalLength >= 0);
    AC_ARRAY_ASSERT(physicalLength <= maxLength());
    if (physicalLength > 0)
                                              ...
                                              const T * pSource, int nCount)
{
    ...
    AC_ARRAY_ASSERT(nCount <= AcArray<T>::maxLength());
    if (bSameBuffer) {
